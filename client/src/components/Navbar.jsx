        ...
        <SearchIcon className='max-md:hidden w-6 h-6 cursor-pointer'/>
       {
         !isLoaded ? null : (
           !user ? (
             <button 
               onClick={() => openSignIn()}
               className="px-4 py-1 sm:px-7 sm:py-2 bg-red-500 hover:bg-red-600 transition rounded-full font-medium cursor-pointer">Login</button>
           ) : (
             <UserButton>
               <UserButton.MenuItems>
                 <UserButton.Action 
                   label="My Bookings" 
                   labelIcon={<TicketPlus width={30} />} 
