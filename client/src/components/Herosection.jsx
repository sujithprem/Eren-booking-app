...
const Herosection = () => {
  ...
  style={{ backgroundImage: `url(${assets.backgroundImage})` }}>
      ...
      <button onClick={()=> navigate('/movies')} className='flex items-center gap-1 px-6 py-3 text-sm bg-red-500 hover:bg-red-600 transition rounded-full font-medium cursor-pointer'>Explore Movies <ArrowRight className='w-5 h-5'/> </button>
    </div>
  )
}
