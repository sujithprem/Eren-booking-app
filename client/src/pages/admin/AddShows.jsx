              ...
              <div className='relative rounded-lg overflow-hidden'>
                ...
                <div className='text-sm flex items-center justify-between p-2 bg-black/70 w-full absolute bottom-0 left-0'>
                  <p className='flex items-center gap-1 text-gray-400'>
                    <StarIcon className='w-4 h-4 text-red-400 fill-amber-300'/>{movie.vote_average.toFixed(1)}
                  </p>
                  <p className='text-gray-300'>{kConverter(movie.vote_count)}Votes</p>
                </div>
              </div>
              {selectedMovie === movie.id && (
                <div className='adsolute top-2 right-2 flex items-center justify-center bg-red-500 h-6 w-6 rounded'><CheckIcon className='w-4 h-4 text-white' strokeWidth={2.5}/></div>
              )}
     ...
     {/* Show Price Input */}
      ...
      <div className='mt-6'>
         ...
         <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-md'>
           ...
           <button onClick={handleDateTimeAdd} className='bg-red-400/80 text-white px-3 py-2 text-sm rounded-lg hover:bg-red-500 cursor-pointer'>Add Time</button>
         </div>
      </div>
            ...
            {Object.entries(dateTimeSelection).map(([date, times])=>(
              <li key={date}>
                ...
                <div className='flex flex-wrap gap-2 mt-1 text-sm'>{times.map((time) => (
                  <div key={time} className='border border-red-400 px-2 py-1 flex items-center rounded'>
                    <span>{time}</span>
      ...
      <button className='bg-red-500 text-white px-8 py-2 mt-6 rounded hover:bg-red-400/90 transition-all cursor-pointer'>Add Show</button>
    </>
  ) : <Loading />
}
